...
def _create_rpc_callback(label, result_counter):
  ...
  def _callback(result_future):
    ...
    if exception:
      ...
      sys.stdout.write('.')
      sys.stdout.flush()
      prediction = result_future.result().outputs['add'].int_val
...
def do_inference(hostport, work_dir, concurrency):
  ...
  inp, res = 1, 11
  request.inputs['a'].CopyFrom(
    tf.contrib.util.make_tensor_proto([inp], dtype=tf.int32, shape=[1,]))
  result_counter.throttle()
  result_future = stub.Predict.future(request, 5.0)  # 5 seconds
  result_future.add_done_callback(
      _create_rpc_callback([res], result_counter))
